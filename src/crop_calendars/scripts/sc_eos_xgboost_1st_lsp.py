...
file_path_ = '/path/to/files'
file_path = os.path.join(file_path_, 'summer_crops_dataset.csv')
results_path = '/path/to/results'
necessary_path = '/path/to/necessary'
sin_features_path = os.path.join(results_path, 'top_feature_sin.csv')
cos_features_path = os.path.join(results_path, 'top_feature_cos.csv')
data_world = os.path.join(necessary_path, 'AgERA5.csv')
rmaskpath = os.path.join(necessary_path, 'ultramonasap_winterwheat_toMaxmask_50km_3857.tif')
mask_template_path = os.path.join(necessary_path, 'worldmask_50km_3857.tif')
mask_path = os.path.join(necessary_path, 'CropCoverFraction_50km_3857.tif')
template_path = os.path.join(necessary_path, 'tftest50km_v9.tif')
gdf_file = os.path.join(necessary_path, 'ne_10m_admin_0_sovereignty/ne_10m_admin_0_sovereignty.shp')
continents_gdf = gpd.read_file(gdf_file)
model_output = '/path/to/models'


seeds = [100, 42, 542, 1000, 232]
for seed in seeds:
    ...
    data.drop(columns='SOS',inplace=True)
    data = data.rename(columns={'EOS':'target'})
    data = data.dropna()
    data['target'] = data['target']*360/365
    data['target'] = np.deg2rad(data['target'])
    # Define the predictors (features for training the model)
    predictors = data.columns.drop({'indekso', 'target', 'x', 'y', 'Legacy'})
    ...
    sin_xgboost_model.save_model(os.path.join(model_output, "s2_eos_xgboost_sin_model.json"))
    ...
    cos_xgboost_model.save_model(os.path.join(model_output, "s2_eos_xgboost_cos_model.json"))
    ...
    joined_df = gpd.sjoin(points_gdf, continents_gdf, how='left', predicate='within')
    ...
    plt.savefig(os.path.join(results_path, "r2validation-s2-eos.png"))
...
with rasterio.open(template_path) as template:
    ...
    transform = Affine(resolution, 0, -180,
                           0, -resolution, 90)
    ...
    with rasterio.open(os.path.join(results_path, "s2_eos_50km.tif"), "w", **profile) as dst:
        reproject(
            source=finalimg,
    ...
    with rasterio.open(os.path.join(results_path, "s2_eos_masked.tif"), "w", **profile) as dst:
        reproject(
            source=finalimgmasked,
            destination=rasterio.band(dst, 1),
            src_transform=template.transform,
            src_crs=template.crs,
            dst_transform=transform,
    ...
    with rasterio.open(os.path.join(results_path, "s2_eos_worldmasked.tif"), "w", **profile) as dst:
        reproject(
            source=finalimgworldmasked,
            destination=rasterio.band(dst, 1),
            src_transform=template.transform,
            src_crs=template.crs,
            dst_transform=transform,
