...
file_path = os.path.join(file_path_, 'winter_crops_dataset.csv')
...
for seed in seeds:
    ...
    sin_xgboost_model.save_model(os.path.join(model_output, "s1_eos_xgboost_sin_model.json"))
    ...
    cos_xgboost_model.save_model(os.path.join(model_output, "s1_eos_xgboost_cos_model.json"))
    ...
    plt.title("Winter crops EOS validation")
...
with rasterio.open(template_path) as template:
    ...
    with rasterio.open(os.path.join(results_path, "s1_eos_50km.tif"), "w", **profile) as dst:
        reproject(
            source=finalimg,
    ...
    with rasterio.open(os.path.join(results_path, "s1_eos_masked.tif"), "w", **profile) as dst:
        reproject(
    ...
    with rasterio.open(os.path.join(results_path, "s1_eos_worldmasked.tif"), "w", **profile) as dst:
        reproject(
