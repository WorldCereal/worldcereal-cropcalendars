...
'''Paths'''
file_path = '/expanded_datasets/summer_expanded_.csv'
results_path = '/prod/outputs/sc_eos'
necessary_path = '/prod/inputs/necessary'
y_pred_path = results_path + '/y_pred.csv'
y_test_path = results_path + '/y_test.csv'
problems_path = results_path + '/sc_eos_problems.csv'
...
plt.savefig(os.path.join(template_results, "r2validation-s2-eos.png"))
...
with rasterio.open(template_path) as template:
    ...
    with rasterio.open(os.path.join(template_results, "s2_eos_50km.tif"), "w", **profile) as dst:
        reproject(
    ...
    with rasterio.open(os.path.join(template_results, "s2_eos_masked.tif"), "w", **profile) as dst:
        reproject(
    ...
    with rasterio.open(os.path.join(template_results, "s2_eos_worldmasked.tif"), "w", **profile) as dst:
        reproject(
